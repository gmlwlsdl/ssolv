...
import PersonaCardSwiper from '@/app/meetings/[token]/result/overview/_components/persona/PersonaCardSwiper';
import OverviewSkeleton from '@/app/meetings/[token]/result/overview/_components/Skeleton';
import SurveyActionButton from '@/app/meetings/[token]/result/overview/_components/SurveyActionButton';
import SurveyStatusBanner from '@/app/meetings/[token]/result/overview/_components/SurveyStatusBanner';
...
const OverviewClient = () => {
  ...
  useEffect(() => {
    if (created) {
      modalHandler.open();
      router.replace(`/meetings/${token}/result/overview`);
    }
  }, [created, token, router, modalHandler]);
